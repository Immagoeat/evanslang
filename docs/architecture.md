...
| `PARSE <target_type>` | Pop a string, convert it to `target_type` (`"int"`, `"float"`, `"bool"`, or `"str"`) via `VM._parse_as` — `str` is the identity; `bool` accepts exactly `"true"`/`"false"`; `int`/`float` use normal numeric parsing. Raises `EvansLangError` if the string doesn't match `target_type`. Push the result. |
