...
around the try body, binding `error.message` into `self.variables` under
`catch_var_name` before running the catch body — Python's own exception
propagation already unwinds through nested `_run_class`/`_execute` calls
(and their `call_depth`-decrementing `finally` blocks) exactly the way the
VM's handler-stack unwind does by hand. `ThrowStatement` evaluates its
expression, checks it's a `str`, and raises `EvansLangError` directly.
