...
1. **Compile each class independently.** Every class's body is compiled
   ...
   known until step 2. While walking a class's own top-level statements
   (not the recursive `_generate_statement` calls for anything nested),
   this pass also records `line_map[statement.line] = <instruction index
   within this class's own block>` for each one, then calls
   `_resolve_gotos(body_instrs, line_map, name)` immediately after —
   see "goto" below for what that does and why it has to happen per-class,
   right here, rather than in the final cross-class patching pass.
...
`goto ln: N;` compiles to a plain `JUMP` — no new opcode was needed, since
it's semantically identical to the relative unconditional jumps `if`/
`while`/`for` already emit internally; the only new work is *resolving*
what relative offset that `JUMP` should carry, which a user-facing line
number makes harder than the compiler's own internally-generated jumps
(which always know their own target immediately, since they're built
bottom-up from already-compiled sub-blocks).

**Two-phase resolution, per class.** `_generate_statement(GotoStatement)`
doesn't know the target's instruction index yet — the target line might
belong to a statement later in the same class body that hasn't been
compiled yet (a forward goto), so it can't be resolved inline the way
every other jump in this codegen is. Instead it emits
`Instruction(OpCode.JUMP, _UnresolvedGoto(target_line))` — a placeholder
operand that is very deliberately not `None`, because `_generate_if`'s own
patch-the-trailing-`JUMP` logic (see "Multi-class codegen" above) also
scans for `operand is None` on `JUMP` instructions it emitted itself;
reusing that sentinel would have the two unrelated "this jump isn't
resolved yet" mechanisms collide. Once a class's `body_instrs` and its
`line_map` (built alongside it — see step 1 above) are both complete,
`_resolve_gotos()` walks `body_instrs` once, finds every `JUMP` still
carrying an `_UnresolvedGoto`, looks its `target_line` up in `line_map`,
and replaces the placeholder with the real relative offset
(`target_index - index`, the same convention every other `JUMP` uses).
A `target_line` missing from `line_map` — no top-level statement in *this*
class starts there — raises `EvansLangError` right here, naming the class,
before `generate()` ever moves on to laying out blocks or patching `CALL`s.
This all happens strictly per-class and before the final cross-class
`block_starts` patching pass, which is what makes "a goto can only target
the same class body it's written in" true almost for free: `line_map` is
never given entries from any other class's statements, so there's no way
for a goto to even see a foreign line number as a candidate, let alone
resolve to one.

**Why goto never desyncs the VM's other jump-based structures.** `if`/
`elseif`/`else`'s `JUMP`/`JUMP_IF_FALSE` pairs, `while`/`for`'s loop-back
`JUMP`, and `try`/`catch`'s `TRY_BEGIN`/`TRY_END` handler-stack bookkeeping
are all naturally exempt from goto interference, but for two entirely
different reasons depending on the structure. `if`/`elseif`/`else`'s own
jumps need no protection at all: a goto landing anywhere inside an `if`'s
compiled body is harmless because an `if` carries no runtime state beyond
what's already on the value stack — there's no counter or handler entry
that a jump could leave stale. `while`/`for`/`try`/`catch` are different:
jumping into or out of one of those *would* leave real state dangling (a
loop's condition/update never re-running correctly, or a `TRY_BEGIN`
handler registration skipped or a `TRY_END` never reached to pop it) — so
those are protected structurally instead, by the parser refusing to parse
a `goto` inside one at all (see the parser row in Modules above) and by
`line_map` only ever containing top-level entries, meaning a nested
target could never be resolved even if a goto instruction somehow
targeted one. Both `if` bodies and top-level statements share one
property that makes them safe: nothing about entering or leaving them
touches any VM-side bookkeeping beyond the value stack, which a bare
`JUMP` never disturbs either way.

**Interpreter.** The VM's `line_map` + relative-offset approach doesn't
translate directly to a tree-walking interpreter, which has no linear
instruction array to jump within — `_run_class` instead iterates
`self.classes[name].body` by index in a plain `while` loop (rather than
Python's `for statement in body`), and `GotoStatement` execution raises a
dedicated `_Goto(target_line)` exception (not an `EvansLangError` — an
internal control-flow signal, the same idea as how the VM's `JUMP` moves
the program counter without going through the error channel at all).
`_Goto` propagates up through however many nested `_execute()` calls sit
between the `goto` and `_run_class`'s own loop (nested if/elseif/else
bodies are just plain Python function calls with no `try/except` around
them, so a raised exception passes straight through), where it's caught,
the target line looked up in a lazily-built-and-cached
`self._line_maps[name]` (same `{line: index}` shape as the VM's
per-class `line_map`, just built once from the already-parsed AST instead
of from compiled instructions), and the loop's index is reassigned before
continuing — an invalid `target_line` raises `EvansLangError` right there,
matching the VM's build-time check as closely as a tree-walker running
statement-by-statement can (this is technically the interpreter's one
first-run-time check where the VM's equivalent is caught before the
program starts at all, since the interpreter has no separate
build/codegen phase to catch it in ahead of time).
