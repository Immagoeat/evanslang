...
`check_element_type()`, and `render_ascii_art()` — logic and state needed
identically by both the VM and the tree-walking `Interpreter`. Both used to carry their own
...
`ASCII_WIDTH` (80) columns, with the row count computed from the source
image's own aspect ratio times `ASCII_HEIGHT_RATIO` (0.55) — this
...
(one byte per pixel, matching `new_width * new_height`) → each byte is
mapped through `ASCII_RAMP` (`"@%#*+=-:. "`, darkest to lightest, 10
characters) via `ramp[len(ramp) - 1 - (pixel * (len(ramp) - 1) // 255)]` —
darker pixels (lower brightness byte) map to denser/darker-looking
characters (`@`) and lighter pixels map toward the empty-looking end
(`.`/space), matching how ASCII art conventionally uses character density
to stand in for grayscale brightness. Rows are joined with `\n` into one
string, which both the VM and interpreter just `print()` directly (not
through `display()` — the string is already fully formatted; `display()`
is for turning an arbitrary evanslang *value* into printable text, and
this is already text).
